...
import { ArrowLeft, User, File as FileIcon, Check, X, Star, ExternalLink } from "lucide-react";
import { getProfileByAaparId, getFiles, subscribe, updateFileStatus } from "@/lib/file-store";
import type { UserProfile, StoredFile } from "@/lib/types";
        ...
        <Accordion type="multiple" className="w-full">
            {documentTypes.map(docType => (
                <AccordionItem value={docType.key} key={docType.key}>
                    <AccordionTrigger className="text-lg font-medium">{docType.name}</AccordionTrigger>
                    <AccordionContent>
                        {files[docType.key] && files[docType.key].length > 0 ? (
                            ...
                            <p className="text-muted-foreground text-center pt-2">No files uploaded for this category.</p>
                        )}
                    </AccordionContent>
                </AccordionItem>
            ))}
